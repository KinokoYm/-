<?php 
  ...
  $GLOBALS['id'] = $_GET['id'];
  //根据id查询当前编辑用户的信息,可以在编辑页面给用户提升为管理员或者用户
  $current_update_user = muzi_connection("select * from users where id ='{$id}'")[0];

  }
  if(!isset($_GET['id'])){

    $current_update_user = null ;
  }
  //获取修改页的页码
  ...
  function update_user(){
    global $user;
    global $current_update_user;
    global $id;

   
    //判断是否输入权限
    if(empty($_POST['slug'])){
      $GLOBALS['message'] = "请选择权限!";
      return;
    }
    ...
    //进行数据的判断
    $info_user = isset($_POST['email']) ? $_POST['email'] : $current_update_user['email'];
    $current_update_user['email'] = $info_user;

    $info_user = isset($_POST['slug']) ? $_POST['slug'] : $current_update_user['slug'];
    $current_update_user['slug'] = $info_user;
    
    $info_user = isset($_POST['nickname']) ? $_POST['nickname'] : $current_update_user['nickname'];
    $current_update_user['nickname'] = $info_user;

    $info_user = isset($_POST['password']) ? $_POST['password'] : $current_update_user['password'];

    $current_update_user['password '] = $info_user;

    //接收最终得到的数据
    $email = $current_update_user['email'];
    $slug = $current_update_user['slug'];
    $nickname = $current_update_user['nickname'];
    $password = $current_update_user['password '];



    $rows = muzi_execute("update users set slug = '{$slug}',email = '{$email}',password = '{$password}',nickname = '{$nickname}'
                                    where id='{$id}';");
        ...
        <div class="col-md-4">
          <form action="<?php echo $_SERVER['PHP_SELF'] ?>?id=<?php echo $id ?>" method="post">
            ...
            <div class="form-group">
              ...
              <input id="email" class="form-control" name="email" type="email" placeholder="邮箱" value="<?php echo $current_update_user['email'] ?>">
            ...
            <div class="form-group">
              ...
              <?php if ($current_user['slug'] == 'superadmin'): ?>
                <select name="slug" id="slug" class="form-control">
                  ...
                  <option value="superadmin" <?php echo $current_update_user['slug'] == 'superadmin' ? ' selected' : ''; ?>>超级管理员</option>
                  <option value="admin" <?php echo $current_update_user['slug'] == 'admin' ? ' selected' : ''; ?>>管理员</option>
                  <option value="user" <?php echo $current_update_user['slug'] == 'user' ? ' selected' : ''; ?>>用户</option>
                </select>                
              <?php else: ?>
                <select name="slug" id="slug" class="form-control">
                  <option value="">请选择权限</option>
            ...
            <div class="form-group">
              ...
              <input id="nickname" class="form-control" name="nickname" type="text" placeholder="昵称" value="<?php echo $current_update_user['nickname'] ?>">
            </div>
            <div class="form-group">
              <label for="password">密码</label>
              <input id="password" class="form-control" name="password" type="text" placeholder="密码" value="<?php echo $current_update_user['password'] ?>">
            </div>
            <div class="form-group">
              <button class="btn btn-primary" type="submit">修改</button>
              <a class="btn btn-default btn-cancel" href="users.php?page=<?php echo $current_update_page ?>">取消</a>
            </div>
          </form>
        </div>
        <div class="col-md-8">
